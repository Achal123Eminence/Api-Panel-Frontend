...
@Component({
  selector: 'app-running-match-market',
  imports: [RouterModule, JsonPipe, CommonModule, FormsModule],
  templateUrl: './running-match-market.html',
  styleUrl: './running-match-market.css',
})
export class RunningMatchMarket {
  ...
  fetchCricketMarketList(id: any) {
    this.isloading = true;
    this.apiService.getRunningEventMarketList({ eventId: id }).subscribe({
      next: (res: any) => {
        this.isloading = false;
        this.cricketMarketList.set(res.data.markets);
        this.eventName = res.data.eventName;
        this.showToast('Market list fetched successfully');
        console.log(this.cricketMarketList());
        this.currentPage.set(1);
      },
      error: (err) => {
        this.isloading = false;
        console.log('Error in getting Market list: ', err);
        this.showToast('Error in getting Market list', true);
      },
    });
  }
  ...
  addSingleMarket(data: any) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        this.isloading = true;
        this.apiService.updateRunningEventMarketStatus(payload).subscribe({
          next: (res: any) => {
            this.isloading = false;
          ...
          error: (err) => {
            ...
            this.showToast(
              `Error in Adding Market : ${err.error.message}`,
              true
            );
            this.fetchCricketMarketList(this.eventId);
          },
